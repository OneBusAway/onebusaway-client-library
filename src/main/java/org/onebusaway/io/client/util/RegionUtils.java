...
public class RegionUtils {
    ...
    public static boolean isRegionUsable(ObaRegion region) {
        if (!region.getActive()) {
        	System.out.println("Region '" + region.getName() + "' is not active.");
            return false;
        }
        if (!region.getSupportsObaDiscoveryApis()) {
        	System.out.println("Region '" + region.getName() + "' does not support OBA Discovery APIs.");
            return false;
        }
     ...
     * resource file
     */
    public synchronized static ArrayList<ObaRegion> getRegions(boolean forceReload) {
        ArrayList<ObaRegion> results = null;
        if (!forceReload) {
            //
            // Check the DB
            //
            results = RegionUtils.getRegionsFromProvider();
            if (results != null) {
            	System.out.println("Retrieved regions from database.");
                return results;
            }
        ...
        if (results == null || results.isEmpty()) {
            ...
            results = RegionUtils.getRegionsFromResources();

            if (results == null) {
                //This is a complete failure to load region info from all sources, app will be useless
            	System.out.println("Regions list retrieved from local resource file was null.");
                return results;
            }

            System.out.println("Retrieved regions from local resource file.");
        } else {
        	System.out.println("Retrieved regions list from server.");
        ...
        RegionUtils.saveToProvider(results);
        return results;
    }

    public static ArrayList<ObaRegion> getRegionsFromProvider() {
...
//        return region;
    ...
    // Saving
    //
    public synchronized static void saveToProvider(List<ObaRegion> regions) {
